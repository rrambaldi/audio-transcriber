...
A folder downloaded by hand has no `config.yaml` in it, and pyannote will not start without one. `audio-transcriber diarize init` writes it, filled in with the model files it actually finds in the folder (and `audio-transcriber diarize check` then runs the same pre-flight the transcription would, without transcribing anything):

```bash
audio-transcriber diarize init      # writes config.yaml in the managed folder
audio-transcriber diarize init "C:/models/pyannote-diar"
audio-transcriber diarize check     # which config it would use, and whether it works
```
